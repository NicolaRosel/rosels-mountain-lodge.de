...
(function () {
  ...
  // Einwilligung gilt max. 12 Monate (DSK-Empfehlung), danach wird erneut gefragt
  var CONSENT_MAX_AGE = 365 * 24 * 60 * 60 * 1000;

  function readConsent() {
    try {
      var raw = JSON.parse(localStorage.getItem(CONSENT_KEY));
      if (raw && raw.value && Date.now() - raw.ts < CONSENT_MAX_AGE) return raw.value;
    } catch (e) {}
  ...
  if (btnAccept) btnAccept.addEventListener('click', () => { saveConsent('granted'); banner.classList.remove('show'); loadGA(); });
  if (btnDeny)   btnDeny.addEventListener('click',   () => { saveConsent('denied');  banner.classList.remove('show'); });
